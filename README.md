...
把 `'你的密码'` 改成你要的密码。**只需改这一处**——前端 `admin.html` 不再硬编码密码（登录时由用户输入、经后端校验），改密码后重新部署即可，前端无需任何改动。

> 安全说明：前端源码中不含任何密码明文；管理员密码仅在登录期间临时保存在浏览器会话（sessionStorage）中，用于调用后端写入接口。切勿把密码写进前端代码。
